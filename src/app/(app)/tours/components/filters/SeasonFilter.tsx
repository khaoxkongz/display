...
function SeasonFilter({
  ...
  const { data: countSpring } = useQuery(
    trpc.tourStudies.countBySeason.queryOptions({ season: "spring" })
  )

  const { data: countSummer } = useQuery(
    trpc.tourStudies.countBySeason.queryOptions({ season: "summer" })
  )

  const { data: countAutumn } = useQuery(
    trpc.tourStudies.countBySeason.queryOptions({ season: "autumn" })
  )

  const { data: countWinter } = useQuery(
    trpc.tourStudies.countBySeason.queryOptions({ season: "winter" })
  )
