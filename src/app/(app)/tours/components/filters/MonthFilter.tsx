...
function MonthFilter({
  ...
  const { data: countJan } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "jan" })
  )

  const { data: countFeb } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "feb" })
  )

  const { data: countMar } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "mar" })
  )

  const { data: countApr } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "apr" })
  )

  const { data: countMay } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "may" })
  )

  const { data: countJun } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "jun" })
  )

  const { data: countJul } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "jul" })
  )

  const { data: countAug } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "aug" })
  )

  const { data: countSep } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "sep" })
  )

  const { data: countOct } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "oct" })
  )

  const { data: countNov } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "nov" })
  )

  const { data: countDec } = useQuery(
    trpc.tourStudies.countByMonth.queryOptions({ monthCode: "dec" })
  )
